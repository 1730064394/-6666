...
class PicoworkersScraper(BaseScraper):
    name = "Picoworkers"
    base_url = "https://www.picoworkers.com"

    async def search(self, keyword: str) -> List[Dict]:
        search_url = f"https://www.picoworkers.com/jobs?q={keyword}"
    ...
    def _parse_task_details(self, content: str, url: str) -> Dict:
        ...
        title = soup.find("h1")
        reward_tag = soup.find("span", class_="job-price")
        time_tag = soup.find("span", class_="job-time")
        
        return {
            "task_id": url.split("/")[-1],
            "title": title.get_text(strip=True) if title else "Unknown Task",
            "description": self._extract_description(soup),
            "reward": float(reward_tag.get_text(strip=True).replace("$", "")) if reward_tag else 1.0,
            "currency": "USD",
            "estimated_time": int(time_tag.get_text(strip=True).replace("min", "")) if time_tag else 10,
            "url": url,
    ...
    def _generate_mock_results(self, keyword: str) -> List[Dict]:
        return [
            {"title": f"{keyword} Social Media Task", "url": f"{self.base_url}/jobs/job1"},
            {"title": f"{keyword} Website Visit", "url": f"{self.base_url}/jobs/job2"},
            {"title": f"{keyword} Quick Survey", "url": f"{self.base_url}/jobs/job3"},
        ]
    ...
    def _generate_mock_task(self, url: str) -> Dict:
        ...
        return {
            "task_id": url.split("/")[-1],
            "title": "Quick Survey - Market Research",
            "description": "Complete a short survey about your preferences.",
            "reward": round(random.uniform(0.5, 3.0), 2),
            "currency": "USD",
